...
fn invalid_fractal(_: f32, _: f32) -> Color {
    Color {r: 0, g: 0, b: 0}
}

fn mandelbrot_set(x: f32, y: f32) -> Color {
    ...
    Color {r: i as u8, g: i as u8, b: clamp_u8(i*i) as u8}
}

fn newton_fractal(x: f32, y: f32) -> Color {
    // based on https://en.wikipedia.org/wiki/Newton_fractal
    fn function(z: Complex<f32>) -> Complex<f32> {
        let mut z3 = z * z * z;
        z3.re -= 1.0;
        z3
    }

    fn derivative(z: Complex<f32>) -> Complex<f32> {
        3.0 * z * z
    }

    fn newton(x: f32, y: f32) -> Color {
        let mut iteration : usize = 1;
        let max_iter : usize = 128;
        let tolerance : f32 = 0.005;
...
fn rescale(point: (usize, usize), shift: (f32, f32), zoom: f32) -> (f32, f32) {
    let (x,y) = point;
    let (shift_x, shift_y) = shift;
    ((x as f32 - shift_x) / zoom, (y as f32 - shift_y) / zoom)
}
...
impl AnimationState {
    pub fn new(size: usize, fractal: &str) -> AnimationState {
        ...
        let mut data : Vec<u8> = vec![255; size * size * COLOR_SIZE];
    ...
    pub fn set_shift_x(&mut self, shift_x: f32) {
    ...
    pub fn set_shift_y(&mut self, shift_y: f32) {
    ...
    pub fn shift_by(&mut self, amount_x: f32, amount_y: f32) {
